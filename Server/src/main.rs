...
async fn connect(req: HttpRequest) -> impl Responder  {
    ...
    let authorized = check_auth(auth);
    unsafe {
        if authorized {
            return ask_connect(req.connection_info().peer_addr().unwrap()).to_string();
        } else {
            println!("{}", style(format!("Access with Pin \"{}\" failed.", req.headers().get("Authorization").unwrap().to_str().unwrap().to_string().replace("Bearer ", ""))).red());
...
#[post("/key")]
async fn key(raw: actix_web::web::Bytes, req: HttpRequest) -> impl Responder  {
    let ip: String = req.connection_info().peer_addr().unwrap().to_string();
    if check_auth(req.headers().get("Authorization"))  && ip_auth(&ip.to_string()) {
        let key = std::str::from_utf8(&raw).unwrap();

        let worked: bool = send_key(key);
        if worked {
            println!("key: {} from {}", style(key).yellow(), style(&ip).green());
            return "true";
        }
        println!("key: {} from {} failed.", style(key).red(), style(&ip).green());
